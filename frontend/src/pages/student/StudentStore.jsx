  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-8 relative z-10">
        <div className="w-20 h-20 bg-slate-50 rounded-[32px] flex items-center justify-center p-4 border border-slate-100 group-hover:bg-orange-50 group-hover:scale-110 transition-all duration-700 shadow-sm relative">
           <div className="absolute inset-0 bg-white/50 backdrop-blur-sm rounded-inherit ring-1 ring-white/20" />
           <img src={brandSymbol} alt="logo" className="w-full h-full object-contain relative z-10" />
        </div>
      ...
      <div className="mb-10 relative z-10 flex-1">
        <div className="inline-flex px-4 py-1.5 bg-orange-50 rounded-full mb-4 border border-orange-100 shadow-sm">
        ...
        <h3 className="text-2xl md:text-3xl font-black text-[#002147] leading-[0.95] tracking-tight uppercase italic mb-6 group-hover:text-orange-600 transition-colors">{c.name}</h3>
...
const StudentStore = () => {
  ...
  const renderBoardGroups = (courseList) => {
    const boards = [...new Set(courseList.map(c => c.board || 'State Board'))];
    const grouped = {};
    boards.forEach(b => {
      const filtered = courseList.filter(c => (c.board || 'State Board') === b);
      grouped[b] = filtered;
    });
        ...
        <div className="bg-white rounded-3xl border border-orange-100 overflow-hidden shadow-sm flex flex-col md:flex-row h-auto min-h-[200px]">
          <div className="w-full md:w-[65%] p-8 flex flex-col justify-center border-r-0 md:border-r-2 border-orange-500 relative overflow-hidden bg-white">
            <div className="relative z-10">
              ...
              <h1 className="text-3xl md:text-5xl font-black text-[#002147] mb-4 italic tracking-tighter leading-none uppercase">
                MYE3-E-LEARNING <span className="text-orange-500 not-italic">STORE</span>
              </h1>
              <p className="text-slate-500 font-bold italic text-sm max-w-xl mb-6 leading-relaxed">Select your class bundle and start learning with our expert-led subject material.</p>
