    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700 pb-10 p-2 md:p-6 lg:p-10 font-sans">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-slate-100 pb-8">
         ...
         <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm w-full sm:w-auto">
               <button 
                 onClick={() => setFilterType('all')}
                 className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filterType === 'all' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}
               >
                 All
               </button>
               <button 
                 onClick={() => setFilterType('Note')}
                 className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filterType === 'Note' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}
               >
                 Notes
               </button>
               <button 
                 onClick={() => setFilterType('Assignment')}
                 className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filterType === 'Assignment' ? 'bg-[#f16126] text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'}`}
               >
                 Tasks
               </button>
            </div>
      ...
      <AnimatePresence mode="popLayout">
        {filteredNotes.length === 0 ? (
          <motion.div 
            ...
            className="py-20 text-center space-y-6 bg-white rounded-[40px] border-2 border-dashed border-slate-100"
          >
             <div className="w-20 h-20 bg-slate-50 text-slate-200 rounded-3xl flex items-center justify-center mx-auto">
                <BookOpen className="w-10 h-10" />
             </div>
             <div className="space-y-1">
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">No Materials Found</h3>
                <p className="text-slate-400 font-bold italic text-xs uppercase tracking-widest">Adjust your search or check your subscriptions.</p>
             </div>
          </motion.div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-6 relative z-10">
                   ...
                   <div className="text-right">
                      <span className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border ${
                        note.type === 'Assignment' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'
                      }`}>
