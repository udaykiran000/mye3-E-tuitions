...
const CourseCard = ({ c, onDetailView }) => {
  ...
  return (
    <motion.div 
      ...
      className="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:border-orange-500/30 transition-all duration-300 p-6 cursor-pointer relative overflow-hidden"
    >
      {/* Brand Bar */}
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center p-3 border border-slate-100 group-hover:bg-orange-50 group-hover:scale-105 transition-all duration-300 shadow-sm">
          <img src={brandSymbol} alt="logo symbol" className="w-full h-full object-contain" />
        </div>
        <div className="text-right">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Tuition Fee</p>
          <p className="text-2xl font-black text-[#002147] italic tracking-tighter leading-none">₹{c.price}</p>
        </div>
      </div>

      <div className="mb-8">
        <p className="text-[10px] font-black text-orange-600 uppercase tracking-[0.3em] leading-none mb-2">Class {c.classLevel.toString().replace(/class/i, '').trim()}</p>
        <h3 className="text-[20px] font-black text-[#002147] leading-tight tracking-tight uppercase italic">{c.name.replace(/Class \d+ - /i, '').replace(/Class \d+/i, '').trim()}</h3>
      </div>

      <div className="flex items-center justify-between -mx-6 -mb-6 mt-6 px-6 py-4 bg-[#002147] group-hover:bg-[#f16126] transition-colors">
        <span className="text-[11px] font-black text-white uppercase tracking-[0.2em] flex items-center gap-2 group-hover:gap-3 transition-all">
           View Details <FiArrowRight className="w-4 h-4" />
        </span>
        <div className="flex -space-x-3">
           <div className="w-7 h-7 rounded-full border-2 border-white bg-white/20 shadow-sm" />
           <div className="w-7 h-7 rounded-full border-2 border-white bg-white/10 shadow-sm" />
           <div className="w-7 h-7 rounded-full border-2 border-white bg-orange-500 flex items-center justify-center text-[10px] font-black text-white shadow-sm">+</div>
        </div>
      </div>
    </motion.div>
  );
...
const StoreCarousel = () => {
  ...
  return (
    <div className="relative w-full h-[200px] md:h-[240px] rounded-3xl bg-transparent flex items-center justify-center overflow-visible">
      {/* Floating Icons with Circular BG - EXACTLY LIKE REFERENCE */}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5 z-30">
        {images.map((_, i) => (
          <div 
            key={i} 
            className={`h-0.5 rounded-full transition-all duration-300 ${i === current ? 'w-6 bg-orange-500 shadow-[0_0_8px_rgba(241,97,38,0.5)]' : 'w-1.5 bg-slate-200'}`}
          />
        ))}
      </div>
    </div>
...
const StudentStore = () => {
  ...
  const filteredCourses = (courses || []).filter(c => 
    ...
    c.classLevel?.toString().toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="bg-[#fcfcfd] min-h-screen font-sans">
      ...
      <div className="bg-[#002147] py-5 px-4 w-full shadow-lg z-20">
        <div className="max-w-[750px] mx-auto flex h-[52px] relative">
          ...
          <button className="bg-orange-500 text-white px-10 h-full text-[13px] font-black uppercase tracking-widest rounded-r-2xl hover:bg-orange-600 shadow-[0_5px_15px_rgba(249,115,22,0.3)] transition-all active:scale-95">
            Search
          </button>
        </div>
      </div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-8 pt-6 pb-10">
        <div className="bg-white rounded-3xl border border-orange-100 overflow-hidden shadow-sm flex flex-col md:flex-row h-auto min-h-[200px]">
          ...
          <div className="w-full md:w-[65%] p-8 flex flex-col justify-center border-r-0 md:border-r-2 border-orange-500 relative overflow-hidden bg-white">
            <div className="absolute top-0 right-0 w-48 h-48 bg-orange-50 rounded-full blur-[80px] -mr-24 -mt-24 opacity-60" />
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-100">
                  <FiCheckCircle className="w-3.5 h-3.5" /> All Subjects
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-blue-100">
                  <FiCheckCircle className="w-3.5 h-3.5" /> Live Classes
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-[35%] bg-slate-50 relative p-6 flex items-center justify-center">
            <StoreCarousel />
          </div>
        </div>
      </div>

      <div className="max-w-[1280px] mx-auto px-4 md:px-8 pb-32 space-y-24">
        {/* Tuitions Section */}
        <section ref={juniorRef} className="scroll-mt-32">
          <div className="flex items-center gap-6 mb-12">
            <div className="px-5 py-2 bg-[#002147] text-white rounded-2xl transform -skew-x-12">
               <h2 className="text-2xl font-black italic tracking-tight uppercase leading-none skew-x-12">School Tuitions</h2>
            </div>
        ...
        <section ref={seniorRef} className="scroll-mt-32">
          <div className="flex items-center gap-6 mb-12">
            <div className="px-5 py-2 bg-orange-500 text-white rounded-2xl transform -skew-x-12">
               <h2 className="text-2xl font-black italic tracking-tight uppercase leading-none skew-x-12">Senior Classes</h2>
            </div>
            <div className="flex-1 h-[2px] bg-slate-100 rounded-full" />
      ...
      <AnimatePresence>
        {showCheckout && selectedCourse && (
          <div className="fixed inset-0 z-[2100] flex items-center justify-center p-4 backdrop-blur-2xl bg-[#002147]/60">
            ...
            >
              <div className="p-10">
                 <h3 className="text-2xl font-black text-[#002147] mb-8 italic tracking-tight uppercase">Checkout Summary</h3>
                 <div className="bg-slate-50 p-6 rounded-3xl mb-8 border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Class Selected</p>
                    <p className="text-xl font-black text-[#002147] italic">{selectedCourse.name}</p>
                 </div>
                 <div className="flex items-center justify-between mb-10 px-4">
                    <span className="text-base font-bold text-slate-500 uppercase tracking-widest">Pricing</span>
                    <span className="text-3xl font-black text-orange-600 italic">₹{selectedCourse.price}</span>
                 </div>
                 <button 
                   disabled={buyLoading}
                   onClick={handlePayment}
                   className="w-full bg-[#002147] text-white py-5 rounded-2xl font-black text-[13px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-orange-600 transition-all shadow-xl active:scale-95 disabled:opacity-50"
                 >
                   {buyLoading ? 'Confirming...' : <>Complete Enrolment <FiArrowRight className="w-5 h-5" /></>}
                 </button>
                 <button onClick={() => setShowCheckout(false)} className="w-full mt-6 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-[#002147] transition-colors">
                    Go Back
                 </button>
              </div>
            </motion.div>
