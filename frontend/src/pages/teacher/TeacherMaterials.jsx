import React, { useState, useEffect } from 'react';
    ...
    const TeacherMaterials = () => {
        ...
        const fetchData = async () => {
          try {
            ...
            if (activeView === 'admin' && aRes.data.length === 0) {
              setAssignments([
                { id: 'mock1', name: 'Class 10 (All Subjects)', classLevel: 'Class 10', subjectName: 'All Subjects', type: 'bundle' },
      ...
      return (
        <div className="space-y-8 md:space-y-10 animate-in fade-in duration-700 pb-20 p-4 md:p-6 font-sans">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
             ...
             <div className="space-y-1 relative z-10">
                <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Study Notes</h1>
                <p className="text-slate-400 text-sm md:text-base font-bold italic">Manage professional notes for your segments.</p>
             </div>
             <button 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
             {materials.length === 0 ? (
               <div className="col-span-full py-16 md:py-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl text-center space-y-4">
                  ...
                  <p className="text-slate-400 font-black text-base md:text-lg italic uppercase tracking-widest px-6">No notes uploaded yet</p>
               </div>
             ) : materials.map((m) => (
               <div key={m._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden group hover:shadow-xl hover:shadow-indigo-900/5 transition-all flex flex-col p-6">
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                     ...
                     >
                        Preview <Download className="w-3 h-3" />
                     </a>
                  </div>
               </div>
             ))}
                     ...
                     <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              ...
                              <p className="text-[11px] font-bold text-slate-400 italic">Target specific segments</p>
                           </div>
                        </div>
                     ...
                     <form onSubmit={handleSubmit} className="space-y-6">
                        {isSuccess ? (
                          ...
                          >
                             ...
                             <h3 className="text-xl font-black text-slate-900 text-center">Note Published!</h3>
                             <p className="text-[11px] text-slate-400 font-bold italic">Adding to your repository...</p>
                          </motion.div>
                        ) : (
                          <>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                               ...
                               {uploadMode === 'upload' ? (
                                 <div className="md:col-span-2 space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2 flex items-center gap-2">
                                       <Upload className="w-3.5 h-3.5 text-emerald-500" /> Select PDF File
                                    </label>
                                    <div className="relative group">
                                       ...
                                       <div className={`w-full px-6 py-6 border-2 border-dashed rounded-2xl transition-all flex flex-col items-center justify-center gap-2 ${selectedFile ? 'border-emerald-500 bg-emerald-50/30' : 'border-slate-200 bg-slate-50 group-hover:border-indigo-600'}`}>
                                          ...
                                          <p className="text-[10px] font-black text-slate-900 uppercase text-center truncate w-[90%]">
                                             {selectedFile ? selectedFile.name : 'Select PDF Note'}
                                          </p>
                                       </div>
                                    </div>
                                 </div>
                               ...
                               <div className="md:col-span-2 space-y-2">
                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2 flex items-center gap-2">
                                     <BookOpen className="w-3.5 h-3.5 text-rose-500" /> Note Category
                                  </label>
                                  <select 
                                    required
                                    value={formData.type}
